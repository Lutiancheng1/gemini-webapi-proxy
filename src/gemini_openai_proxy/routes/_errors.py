...
from gemini_openai_proxy.client.pool import invalidate_client
from gemini_openai_proxy.config import Settings
from gemini_openai_proxy.errors import CookieExpiredError, ImageExportError, UpstreamRefusalError
from gemini_openai_proxy.schemas import ErrorResponse
from gemini_openai_proxy.utils.upstream_refusal import (
...
async def map_api_error(exc: Exception, settings: Settings) -> JSONResponse:
    ...
    if isinstance(exc, UpstreamRefusalError):
        return error_json(f"Upstream refused request: {exc}", exc.http_status)
    ...
    if isinstance(exc, RuntimeError):
        ...
        if looks_like_image_refusal(text) or looks_like_upstream_refusal(text):
            return error_json(
                f"Upstream refused request: {text}",
                status=403,
            )
